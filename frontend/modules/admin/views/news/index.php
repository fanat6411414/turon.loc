...
<div class="book-index card card-body">
    ...
    <div class="row">
        ...
        <div class="col-md-6">
            <p class="text-right">
                <?= Html::a('<i class="fa fa-plus-circle"></i>', ['/news/add'], ['class' => 'btn btn-sm btn-success']) ?>
                <?= Html::submitButton('<i class="fa fa-search"></i>', ['class' => 'btn btn-sm btn-primary']) ?>
                <?= Html::a('<i class="fa fa-filter"></i>', ['/news/index'], ['class' => 'btn btn-sm btn-default']) ?>
            </p>
        </div>
    </div>
    <?php ActiveForm::end(); ?>
    <?= GridView::widget([
        ...
        'columns' => [
            ...
            [
                'attribute' => 'image',
                'value' => function ($model){ return Html::img($model->img->getUrl(), ['width' => '50px']); },
                'options' => ['style' => 'width: 50px'],
                'format' => 'raw'
            ],
            [
                'attribute' => 'name',
                'value' => function ($model){ return Html::a($model->name, ['/news/edit', 'id' => $model->alias]); },
                'format' => 'raw'
            ],
            [
                'attribute' => 'news_type',
                'value' => function ($model){ return $model->newsType->name; },
            ...
            [
                ...
                'buttons' => [
                    '/news/translate' => function ($url, $model) {
                        return Html::a('<i class="fa fa-language"></i>', ['/news/translate', 'id' => $model->alias],
                            ['class' => 'btn btn-sm btn-primary', 'style' => 'margin-left: 5px']);
                    },
                    'delete' => function ($url, $model) {
                        return Html::a(
                            '<i class="fa fa-trash"></i>', ['/news/delete', 'id' => $model->alias], [
